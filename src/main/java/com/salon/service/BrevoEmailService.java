...
public class BrevoEmailService {
    ...
    public void sendBookingCancellation(Booking booking, User customer) {
        ...
        try {
            ...
            String jsonPayload = String.format(
                    "{\"sender\":{\"email\":\"%s\"},\"to\":[{\"email\":\"%s\",\"name\":\"%s\"}],\"subject\":\"Booking Cancelled - Salon Booking System\",\"textContent\":\"%s\"}",
                    fromEmail,
                    customer.getEmail(),
                    customer.getFullName(),
                    escapedContent
            ...
            if (responseCode >= 200 && responseCode < 300) {
                ...
                System.err.println("❌ Cancellation email failed with code: " + responseCode);
                try (java.util.Scanner s = new java.util.Scanner(conn.getErrorStream(), "UTF-8").useDelimiter("\\A")) {
                    System.err.println("Error: " + (s.hasNext() ? s.next() : ""));
                }
            }
